...
# What no API can do
#
# GitHub exposes no endpoint for either of these — a PATCH carrying
# avatar_url or open_graph_image_url is accepted and silently ignored — so
# they are printed rather than pretended.
# --------------------------------------------------------------------------
step "Only a person can do these"
# No detection here on purpose. A custom avatar is served from the same URL
# as the default one, so the first version of this check reported "still the
# default" for an avatar that had just been set — and a check that reports
# falsely is worse than no check.
skip "organisation avatar — https://github.com/organizations/${ORG}/settings/profile"
note "docs/brand/remedik-mark.png, if it is still the generated one"
note "the mark rather than the avatar exports: those carry the wordmark, unreadable at the 40px GitHub renders"
skip "repository social preview — https://github.com/${REPO}/settings"
note "docs/brand/remedik-banner.png, 1280x640, exactly the ratio GitHub asks for"
