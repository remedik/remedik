...
| crdCheck | object | `{"enabled":true}` | Refuse an upgrade whose CRDs the cluster does not have.  `helm upgrade` does not upgrade CRDs: Helm applies a chart's crds/ directory once, on first install, and never again. So a field added since your install is pruned silently by the API server rather than rejected — a strategy that asks for human approval can lose the field that makes it wait. The chart compares a stamp in each generated CRD with the one in the cluster and stops with the command that fixes it.  Turn it off if you manage CRDs yourself and would rather not be asked. |
| crdCheck.enabled | bool | `true` | Compare the cluster's CRDs with the chart's before upgrading. |
...
| dashboard.links | list | `[]` (no links) | Links out of a remediation's page, as `name` and `url`. The URL is a template over `{namespace}`, `{target}`, `{name}`, `{alert}`, `{fingerprint}`, `{from}` and `{to}` — the last two an ISO window around the record, padded either side. Values are percent-encoded on substitution, and a template whose scheme is not `http` or `https` is refused at startup with a log line naming it. A fingerprint printed as text is a dead end; this is what turns it into a click. |
...
| workloadAlerts.additionalLabels | object | `{}` | Labels this PrometheusRule needs to be selected. Defaults to `prometheusRule.additionalLabels`, because it is the same label for the same reason and setting it twice is how a rule file ends up created and never loaded — with nothing anywhere reporting it. |
...
| workloadAlerts.crashLoopThreshold | int | `2` | How many restarts inside that window count as a crash loop. Two is a container that died twice, which no healthy workload does. |
| workloadAlerts.crashLoopWindow | string | `"10m"` | The window the restart count is measured over.  The crash-loop rules count restarts rather than reading `kube_pod_container_status_waiting_reason`, which is the obvious metric and the wrong one to depend on: kube-state-metrics 2.20 stopped publishing it, so a rule written on it cannot fire at all on a current cluster and nothing anywhere says so. Restarts have been in every version and are not going away.  Do not shorten this below 10m without also dropping the threshold to 1. Kubernetes backs a crash loop off exponentially, capped at five minutes between restarts, so a settled CrashLoopBackOff produces very few restarts per window. Measured on a workload that had been failing for thirteen minutes: 0 restarts in the last 3m, 1 in 5m, 2 in 10m. A 3m window with a threshold of 2 can therefore never fire, which looks exactly like remedik being broken. |
